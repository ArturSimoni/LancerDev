        ...
        <div style={s.heroInner}>
          ...
          <div style={s.heroCtas}>
            {isLoggedIn ? (
              ...
              <>
                <Link to="/cadastro" style={s.btnPrimary}>Criar conta grátis</Link>
      ...
      <section style={s.section}>
        ...
        {loading ? (
          ...
          <div style={s.emptyState}>
            ...
            <Link to="/criar-projeto" style={s.btnPrimary}>Publicar o primeiro projeto</Link>
          </div>
        ) : (
          <div style={s.grid}>
            {projects.map((project) => (
              <div key={project.id} style={s.card}>
                <div style={{ flex: 1 }}>
                  ...
                  <h3 style={{ ...s.cardTitle, marginTop: '10px' }}>{project.title}</h3>
                  <p style={s.cardDesc}>
                    {project.description.length > 160
                      ? `${project.description.substring(0, 160)}...`
                      : project.description}
                  </p>
                  <span style={{ ...s.clientName, display: 'block', marginTop: '10px' }}>
                    Por: {project.client?.name}
                  </span>
                </div>
                ...
                <div style={{ flexShrink: 0 }}>
                  <button
                    type="button"
                    onClick={() => isLoggedIn ? navigate(`/projeto/${project.id}`) : navigate('/login')}
                    style={s.btnOutline}
                    onMouseEnter={e => { e.target.style.backgroundColor = '#ff6b00'; e.target.style.color = '#000'; }}
...
const s = {
  ...
  hero: { padding: '80px 0 60px', borderBottom: '1px solid #222', marginBottom: '60px' },
  ...
  // CORRIGIDO: layout retangular/horizontal
  grid: { display: 'flex', flexDirection: 'column', gap: '16px' },
  card: { backgroundColor: '#111', border: '1px solid #222', borderRadius: '10px', padding: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px' },
  cardTop: { display: 'flex', alignItems: 'center', gap: '12px' },
  cardTitle: { fontSize: '17px', fontWeight: '600', margin: '0 0 4px', lineHeight: '1.3' },
  cardDesc: { color: '#777', fontSize: '13px', lineHeight: '1.6', margin: '0' },
  cardFooter: { display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px', flexShrink: 0 },
  ...
  btnOutline: { backgroundColor: 'transparent', border: '1px solid #ff6b00', color: '#ff6b00', padding: '10px 18px', borderRadius: '5px', cursor: 'pointer', fontWeight: '500', fontSize: '13px', whiteSpace: 'nowrap' },
