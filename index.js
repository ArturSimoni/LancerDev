...
app.get('/projects', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT p.id, p.title, p.description, p.budget, p.status, p.created_at, u.name AS client_name, u.email AS client_email
       FROM projects p
...
app.post('/projects', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO projects (client_id, title, description, budget) VALUES ($1, $2, $3, $4) RETURNING id, title, description, budget, status, created_at',
...
app.get('/projects/:id/proposals', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      `SELECT pr.id, pr.amount, pr.cover_text, pr.status, pr.created_at,
